...
#'
#' @import CEGO
algorithm_lassoSMBO <- function(modelOptimizer,
                           distanceFunction, designGenerator,
                           budget = 100,
                           useBP = F, useEI = F){
    lassoSmbo <- function(job, data, instance){
        origDF <- distanceFunction
        ...
        while((length(y) + pointsPerIter) <= budget){
            ...
            saveRDS(list(modelY,modelX),file = "someResult.rds")

            lassoWeights <- adaptive_Lasso(modelX,modelY)
            ...
            for(i in 1:length(infillCriteria)){
                modelOptima[i,] <- modelOptimizer(fun = infillCriteria[[i]])
            }
